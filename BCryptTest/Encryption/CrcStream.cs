...
namespace TicketmasterSystems.Encryption
{
	...
	{
		public static string GetFileCRC(FileInfo file)
		{
			if(file == null || !file.Exists) return 0.ToString("X8");

			using(FileStream fs = file.OpenRead())
			using(CrcStream strm = new CrcStream(fs))
			using(StreamReader rdr = new StreamReader(strm))
			{
				rdr.ReadToEnd();
				return strm.ReadCrc.ToString("X8");
			}
		}
